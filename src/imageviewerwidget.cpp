...
void ImageViewerWidget::onGradeComic() {
    // Launch external grading application via batch script
    QString rootDir = QCoreApplication::applicationDirPath() + "/../../";
    QString launchPath = rootDir + "launch.bat";

    if (!QFileInfo::exists(launchPath)) {
        QMessageBox::warning(this, "Launcher Not Found", QString("Could not find launcher at:\n%1").arg(launchPath));
        return;
    }

    // Use cmd.exe to start the .bat in a new window
    bool ok = QProcess::startDetached(
        "cmd.exe",
        QStringList() << "/c" << "start" << "" << QDir::toNativeSeparators(launchPath),
        QDir::toNativeSeparators(rootDir)
    );

    if (!ok) {
        QMessageBox::critical(this, "Launch Failed", "Failed to start the ComicVision grading application.");
        return;
    }

    // Optional: notify user and keep internal panel hidden
    if (resultsEdit && resultsEdit->parentWidget()) {
        resultsEdit->parentWidget()->hide();
    }
}
